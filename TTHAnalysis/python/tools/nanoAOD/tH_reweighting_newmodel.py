...
class TH_weights( Module ):
    def __init__(self, process):
        ...
        #CP-odd: cosa =0 kW=0 ktt=1
        outn="param_card_CP_odd.dat"
        kt = 3./2 #CP-odd ktt sin(a) = 1 
        kV = 0.0
        cosaA=0
        out=template.format(khtt=kt,katt=2.*kt/3,cosa=cosaA,kSM=kV)
        ...
        kt = 1.0
        kV = 0.0
        outn="param_card_kV_%s_kt_%s.dat"%(numToString(kV), numToString(kt))
        out=template.format(khtt=kt,katt=2.*kt/3,cosa=1,kSM=kV)
        outf=open('%s/%s'%(self.tmpdir,outn),'w')
        outf.write(out)
        outf.close()
        self.param_cards.append('%s/%s'%(self.tmpdir,outn))
        '''
        for kV, kt in zip(kVkV, ktkt):
            outn="param_card_kV_%s_kt_%s.dat"%(numToString(kV), numToString(kt))
            out=template.format(khtt=kt,katt=2.*kt/3,cosa=1,kSM=kV)
            outf=open('%s/%s'%(self.tmpdir,outn),'w')
            outf.write(out)
